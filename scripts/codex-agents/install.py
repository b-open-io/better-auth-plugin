#!/usr/bin/env python3
"""Install, check, update, or uninstall Siggy's generated Codex adapter."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if manifest.get("manager") != MANAGER or [a.get("generated_file") for a in manifest.get("agents", [])] != [AGENT_FILE]:
        print("error: invalid or missing Sigma Auth agent manifest")
        return 1
    ...
    if ownership_path.is_symlink():
        print(f"error: refused symlinked Sigma Auth ownership metadata: {ownership_path}")
        return 1
    ownership = load_json(ownership_path, {"manager": MANAGER, "agents": {}})
    if ownership.get("manager") != MANAGER:
        print(f"error: invalid Sigma Auth ownership metadata: {ownership_path}")
        return 1
    record = ownership.get("agents", {}).get(AGENT_FILE)

    if args.uninstall:
        if not record:
            print(f"unchanged: {AGENT_FILE} is not managed by Sigma Auth")
            return 0
        present = destination.exists() or destination.is_symlink()
        modified = present and (
            destination.is_symlink()
            or not destination.is_file()
            or sha256_file(destination) != record.get("hash")
        )
    ...
    print("Start a new Codex session before invoking sigma_auth_guide.")
    return 0
