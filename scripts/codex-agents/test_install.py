#!/usr/bin/env python3
"""Regression tests for the Sigma Auth Codex adapter and installer."""
...
INSTALLER = ROOT / "scripts" / "codex-agents" / "install.py"
GENERATOR = ROOT / "scripts" / "codex-agents" / "generate.py"
AGENT = "sigma-auth-guide.toml"
...
class InstallerTests(unittest.TestCase):
    ...
    def run_installer(self, *args: str, env: dict[str, str] | None = None) -> subprocess.CompletedProcess[str]:
        return subprocess.run(
            [sys.executable, str(INSTALLER), "--plugin-root", str(ROOT), *args],
            cwd=self.base,
            env=env,
            text=True,
            capture_output=True,
    ...
    def test_generated_toml_parses_and_preserves_canonical_body(self) -> None:
        ...
        raw = (ROOT / "agents" / "sigma-auth-guide.md").read_text(encoding="utf-8")
        body = raw.split("\n---\n", 1)[1]
        self.assertEqual(parsed["name"], "sigma_auth_guide")
        self.assertTrue(parsed["developer_instructions"].endswith("\n" + body))
        self.assertIn("Codex compatibility prelude (Sigma Auth)", parsed["developer_instructions"])
    ...
    def test_update_replaces_unmodified_managed_file(self) -> None:
        ...
        ownership_path = target_dir / ".sigma-auth-agents.json"
        ownership = json.loads(ownership_path.read_text())
        old = 'name = "sigma_auth_guide"\n'
    ...
    def test_unmanaged_collision_refused_and_force_quarantines(self) -> None:
        ...
        recovered = list((target_dir / ".sigma-auth-agents-trash").glob(f"{AGENT}*"))
    ...
    def test_uninstall_broken_managed_symlink_requires_force(self) -> None:
        self.assertEqual(self.run_installer().returncode, 0)
        target_dir = self.base / ".codex" / "agents"
        target = target_dir / AGENT
        target.unlink()
    ...
    def test_symlinked_ownership_metadata_is_refused(self) -> None:
        target_dir = self.base / ".codex" / "agents"
        target_dir.mkdir(parents=True)
        external = self.base / "ownership.json"
        external.write_text('{"manager":"sigma-auth","agents":{}}\n')
        (target_dir / ".sigma-auth-agents.json").symlink_to(external)
        result = self.run_installer()
        self.assertEqual(result.returncode, 1)
        self.assertIn("refused symlinked", result.stdout)
        self.assertEqual(external.read_text(), '{"manager":"sigma-auth","agents":{}}\n')
