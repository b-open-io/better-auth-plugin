			...
			// The auth server's getAdditionalUserInfoClaim looks up selectedBapId and fetches BAP profile
			{
				...
				handler: createAuthMiddleware(async (ctx) => {
					...
					try {
						...
						console.log(
							`✅ [OAuth Consent Hook] Stored BAP ID in consent: user=${session.user.id.substring(0, 15)}... bap=${selectedBapId.substring(0, 15)}...`,
						);
					} catch (error) {
						console.error(
		...
		before: [
			{
				...
				handler: createAuthMiddleware(async (ctx) => {
					...
					if (grantType === "authorization_code") {
						...
						console.log(
							`✅ [OAuth Token] Client authenticated via Bitcoin signature (clientId: ${clientId})`,
						);
					...
					if (grantType === "refresh_token") {
						...
						console.log(
							`✅ [OAuth Token Refresh] Client authenticated via Bitcoin signature (clientId: ${clientId})`,
						);
